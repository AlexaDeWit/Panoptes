...
Partly held. `pnpm-workspace.yaml` sets `minimumReleaseAge` to 10,080 minutes and `renovate.json5` sets the same seven days with `internalChecksFilter: 'strict'`, so a freshly published version neither resolves nor is proposed until it has aged, and the quarantine lifts only for an OSV vulnerability fix, where waiting is the greater risk. Provenance is checked on top of that. `scripts/check-provenance.mjs` verifies each catalog package's npm attestation against the sigstore trust root and fails where one that verified before no longer does, or where a signature or an attestation does not verify at all. `dependency-provenance.txt` is the committed record it compares against, so the loss is a red gate rather than a thing nobody notices. It runs in the CI gate on every pull request, which is where a Renovate bump appears, and `docs/release.md` names it as a step before the tag. Three residuals stand. Two thirds of the catalog publish an attestation and the rest publish none, and for those the quarantine is the only control there is; the check prints that list on every run. An attestation names the repository and the workflow a release was built from rather than vouching for what is in it, so an attacker who reaches the source publishes with valid provenance and passes the check. And the seven-day window is unchanged: a release nobody catches inside it still arrives. Actions are pinned to a commit SHA under zizmor's hash-pin policy and nothing checks an attestation of theirs. The severity stays high because none of this narrows the blast radius of what does get through, which runs in the build and ships in a binary.
