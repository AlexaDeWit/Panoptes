...
Partly held. `pnpm-workspace.yaml` sets `minimumReleaseAge` to 10,080 minutes and `renovate.json5` sets the same seven days with `internalChecksFilter: 'strict'`, so a freshly published version neither resolves nor is proposed until it has aged, and the quarantine lifts only for an OSV vulnerability fix, where waiting is the greater risk. What it does not cover is a release nobody catches inside those seven days, and no check of provenance or attestation exists at all: not npm provenance statements, not a sigstore or SLSA attestation, and no gate asking a person to read a dependency diff. Issue #113 is where that work lives. The severity stays high because the quarantine narrows the window rather than the blast radius, and what gets through runs in the build and ships in a binary.
